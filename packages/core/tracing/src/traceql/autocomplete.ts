...
import type { Situation } from '../traceql/situation'
import { getSituation } from '../traceql/situation'
...
export class CompletionProvider implements Monaco.languages.CompletionItemProvider {
  ...
  private async getTagValues(tagName: string, query: string): Promise<Array<SelectableValue<string>>> {
    ...
    if (Object.prototype.hasOwnProperty.call(this.cachedValues, cacheKey)) {
 ...
 * @param monaco
 */
function getMonacoCompletionItemKind(type: CompletionType, monaco: typeof Monaco): Monaco.languages.CompletionItemKind {
  switch (type) {
    case 'TAG_NAME':
...
type Completion = {
  ...
  label: string;
  insertText: string;
  insertTextRules?: Monaco.languages.CompletionItemInsertTextRule; // we used it to position the cursor
  documentation?: string | IMarkdownString;
  detail?: string;
}
...
function getRangeAndOffset(monaco: typeof Monaco, model: Monaco.editor.ITextModel, position: Monaco.Position) {
